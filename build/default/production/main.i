...
unsigned char arr[9][3] = {"ON", "GN", "G1", "G2", "G3", "G4", "G5", "GR", "C "};
unsigned char i = 0, rpm, clear = 0, count = 0;
unsigned char choose_flag = 0;
unsigned char password[5], clock_reg[3];
unsigned char password1[5] = "1111";
unsigned char menu_array[6][19] = {"View log        ", "download log     ", "download log    ", "Clear log       ", "Set time ", "Change password    "};
unsigned char dashboard = 0, enter_password = 0, view_menu = 0, change_menu = 0, setting_log = 0;
unsigned char log_flag;
unsigned char logs[10][17], log, store_data[11];
unsigned char view_log = 0, download_log = 0, clear_log = 0, set_time = 0, change_password = 0, store_event = 0, get_time = 0;
        ...
        if (dashboard)
{
    ...
    rpm = ((read_adc(0x04)) / 10.33);
    clcd_putch(rpm / 10 + '0', (0xC0 + (14)));
    clcd_putch(rpm % 10 + '0', (0xC0 + (15)));
       ...
       if (enter_password)
{
    ...
    while (1)
    {
        ...
        {
            if (input == 0)
                clcd_print("_ENTER_PASSWORD ", (0x80 + (0)));
            ...
            else if (input == 4)
            {
                ...
                if (!strcmp(entered, password1))
                {
                    clcd_write(0x01, 0);
                    clcd_print("ENTERED_PASSWORD", (0x80 + (0)));
        ...
        else if (attempts == 0)
        {
            ...
            clcd_print("wait two min", (0xC0 + (0)));
            for (delay = 250000; delay--;)
            {
                if (delay == 1)
                {
                    delay = 250000;
                    clcd_putch(count / 100 + '0', (0xC0 + (13)));
                    clcd_putch((count / 10) % 10 + '0', (0xC0 + (14)));
                    clcd_putch(count % 10 + '0', (0xC0 + (15)));
                    count++;
                    if (count == 121)
                    {
                        clcd_write(0x01, 0);
        ...
        if (view_menu)
{
    ...
    clcd_print(menu_array[menu], (0x80 + (1)));
    clcd_print(menu_array[menu + 1], (0xC0 + (1)));
...
if (change_menu)
{
    ...
    if (t_count1 < 500 && t_count1 > 0 && key2 == 0xFF)
    {
        clcd_write(0x01, 0);
        clcd_putch(' ', (0x80 + (0)));
        clcd_putch('*', (0xC0 + (0)));
    ...
    else if (t_count1 > 500 && key2 == 0xFF)
    {
        choose_flag = 3;
        t_count1 = 0;
        clcd_write(0x01, 0);
    ...
    if (t_count2 < 500 && t_count2 > 0 && key2 == 0xFF)
    {
        clcd_write(0x01, 0);
        clcd_putch('*', (0x80 + (0)));
        clcd_putch(' ', (0xC0 + (0)));
    ...
    else if (t_count2 > 500 && key2 == 0xFF)
    {
        t_count2 = 0;
        choose_flag = 0;
        clcd_write(0x01, 0);
...
if(view_log)
{
    ...
    if (clear == 0)
    {
        ...
        clcd_putch(log + 48, (0xC0 + (0)));
        clcd_print(logs[log], (0xC0 + (1)));
    }


    if (clear == 1)
    {
        clcd_print("LOGS_ARE_CLEARED", (0x80 + (0)));
        clcd_print("               ", (0xC0 + (0)));
    ...
    if (view_select1 < 200 && view_select1 > 0 && view_key == 0xFF)
    {
        view_select1 = 0;
        if (log <= 9)
            log++;
    }
    else if (view_select1 > 200 && view_key == 0xFF)
    {
        view_select1 = 0;
    }
    if (view_select2 < 200 && view_select2 > 0 && view_key == 0xFF)
    {
        view_select2 = 0;
        if (log >= 1)
            log--;
    }
    if (view_select2 > 200 && view_key == 0xFF)
    {
        clcd_write(0x01, 0);
        choose_flag = 2;
        view_select2 = 0;
...
if (change_password)
{
    ...
    else if (index1 == 4)
    {
        ...
        else if (index2 == 4)
        {
            ...
            {
                clcd_write(0x01, 0);
                clcd_print("SET PASSWORD", (0x80 + (0)));
                clcd_print("SUCCESSFUL", (0xC0 + (0)));
...
if (download_log)
{
    ...
    if (clear == 0)
    {
        ...
        clcd_print("                ", (0x80 + (1)));
        clcd_print("DOWNLOAD COMPLETE", (0xC0 + (0)));
        for (delay = 800000; delay--;);
        clcd_write(0x01, 0);
        choose_flag = 2;
    }
    ...
    {
        clcd_print("LOGS ARE CLEARED", (0x80 + (1)));
        for (delay = 800000; delay--;);
        clcd_write(0x01, 0);
        choose_flag = 2;
    }
...
if (set_time)
{
    ...
    unsigned char set_time_key = read_switches(0);
    ...
    if (val1 < 200 && val1 > 0 && set_time_key == 0xFF)
    {
        val1 = 0;
        change++;
        change = change % 3;
    }
    else if (val1 > 200 && set_time_key == 0xFF)
    {
        val1 = 0;
        choose_flag = 2;
    }
    if (val2 < 200 && val2 > 0 && set_time_key == 0xFF)
    {
        val2 = 0;
        if (change == 0)
        {
    ...
    else if (val2 > 200 && set_time_key == 0xFF)
    {
        if (val2 > 200)
        {
            val2 = 0;
            clcd_print("SET TIME SUCCESS", (0x80 + (0)));
            clcd_print("                  ", (0xC0 + (0)));
